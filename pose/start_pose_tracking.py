...
def print_header():
    ...
    print(f"{Colors.GREEN}   （基于YOLOv8 Pose姿态估计）{Colors.NC}")
    print(f"{Colors.GREEN}{'='*40}{Colors.NC}")
    print()
...
def check_files():
    """检查必需文件"""
    if not Path('pose_head_tracking.py').exists():
        print(f"{Colors.RED}错误: 找不到 pose_head_tracking.py{Colors.NC}")
        sys.exit(1)
    ...
    if not Path('yolov8_pose.rknn').exists():
        print(f"{Colors.YELLOW}警告: 找不到 yolov8_pose.rknn 模型文件{Colors.NC}")
        print()
...
def main():
    ...
    try:
        # 启动检测程序
        print(f"{Colors.GREEN}[1/2] 启动 YOLOv8 Pose 检测程序...{Colors.NC}")
        detect_cmd = [
        ...
        detect_log = open('logs/pose_detection.log', 'w')
